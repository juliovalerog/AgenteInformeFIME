...
def generate_portfolio_suggestion(df: pd.DataFrame, model: str, timeout_s: int = 180) -> str:
    prompt = (
        ...
        "Tarea: selecciona exactamente 8 valores y asigna pesos exactos.\n"
        "Reglas de consistencia (obligatorias):\n"
        "- Usa exactamente los mismos 8 tickers en todas las secciones.\n"
        "- Tabla de pesos debe tener 8 filas (una por ticker) y una fila TOTAL.\n"
        "- La suma de pesos debe ser exactamente 100% y ningun valor >20%.\n"
        "- No uses datos externos ni lenguaje concluyente.\n"
        "Formato estricto:\n"
        "A) Tabla Seleccion (8 filas): | Ticker | Sector | Return_Pct | Vol_Pct | Max_Drawdown_Pct | Score |\n"
        "B) Tabla Pesos (9 filas): | Ticker | Peso (%) | y ultima fila TOTAL = 100%.\n"
        "C) Sintesis final (3-5 lineas) con trade-offs y limitaciones.\n\n"
        "Tabla completa:\n"
